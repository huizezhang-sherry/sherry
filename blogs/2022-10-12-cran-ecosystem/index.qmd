...
summary: ''
authors: []
lastmod: '2022-11-18T22:49:21+10:00'
featured: no
projects: []
freeze: true
---
...
save(raw, file = here::here("content/post/2022-10-12-cran-ecosystem/raw.rda"))
```
...
```{r eval = FALSE, echo=TRUE}
#| code-fold: true
find_all_deps <- function(from, data){
  print(from)
  dt <- tibble()
  dt2 <- data
  i <- 1
  while(nrow(dt2) > nrow(dt)){
    ...
    dt2 <- dt %>% 
      rename(from = to) %>% 
      left_join(dep_lookup_tbl %>% select(-type), by = "from") %>% 
      rename(!!quo_name(n) := from)
    i <- i + 1
  }
  
  dep <- dt2 %>%
    pivot_longer(
      cols = c(contains("from"),  "to"),
      names_to = "dump", values_to = "to") %>%
    distinct(to) %>%
    filter(!is.na(to)) %>%
    mutate(to = sort(to))
  return(dep)
...
dep_all <- dep_lookup_tbl %>% 
  arrange(-desc(from)) %>% 
  nest(direct_deps = -from) %>% 
  mutate(all_deps = map2(from, direct_deps, find_all_deps))
...
nodes <- tibble(id = unique(c(edges$from, edges$to))) %>% 
  left_join(edges %>% count(from, name = "n_revdep"), by = c("id" = "from")) %>% 
  left_join(edges %>% count(to, name = "n_dep"), by = c("id" = "to")) %>% 
  filter(!is.na(id)) %>% 
...
################################################################
p <- nodes %>% 
  mutate(r_dep = rank(n_dep, ties.method = "first"),
         r_revdep = rank(n_revdep, ties.method = "first"),
         tooltip = glue::glue("Pkg: {id}, dep: {n_dep}, revdep: {n_revdep}")) %>% 
  ...
  scale_x_continuous(limits = c(0,250), breaks = c(0, 1, 5, 10, 20, 40, 80, 120, 160, 200)) + 
...
tgt_pnts <- prct_tbl %>% 
  ...
  filter(p %in% c(0.9, 0.95, 0.99, 0.995, 0.999) |  p < 0.9 | prct == 1 | n == 5) %>% 
  group_by(p) %>% 
  filter(n == min(n))
